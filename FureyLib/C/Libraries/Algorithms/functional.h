 ...
 */\
static inline T *map_##T(size_t n, const T *array, T(*transform)(T elem), T *out) {\
    assert(array != NULL);\
 ...
 */\
static inline size_t filter_##T(size_t n, const T *array, bool(*predicate)(T elem), T *out) {\
    assert(array != NULL);\
 ...
 */\
static inline T reduce_##T(size_t n, const T *array, T(*accumulator)(T acc, T elem), T start) {\
    assert(array != NULL);\
 ...
 */\
static inline bool foreach_##T(size_t n, const T *array, bool(*action)(T elem), T *out) {\
    assert(array != NULL);\
    assert(action != NULL);\
    for (size_t i = 0; i < n; ++i) {\
        if (!action(array[i])) {\
 ...
 * Returns <out>. <out> cannot be NULL.\
 */\
static inline T *reverse_##T(size_t n, const T *array, T *out) {\
    assert(array != NULL);\
    assert(out != NULL);\
    if (array != out) {\
        for (size_t i = 0; i < n; ++i) {\
