...
{
	if (!IsValid(Instance) || !IsValid(Instance->PlayerActor))
	{
		return false;
	}
...
{
	if (!IsValid(Instance) || !IsValid(Actor))
	{
		return false;
	}
...
{
	if (!IsValid(Instance) || !IsValid(Instance->PlayerActor) || !IsValid(Actor))
	{
		return false;
	}
...
{
	if (IsValid(Instance) || !IsValid(GWorld))
	{
		return nullptr;
	}
